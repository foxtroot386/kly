...
<body style='margin: 0;'>
	<div class='container-fluid'>
		...
		@if(isset($notice))
		<div class='bg-success text-light text-center'>{!!$notice!!}</div>
		@endif
		<form method='post' action="{{route('home.store')}}" class='container mt-5'>
			{{csrf_field()}}
			<div class='form-group'>
				<label for='name'>Name<b class='text-danger'>*</b></label>
				<input type="text" id='name' name="name" class='form-control' placeholder='Your Name Here' value="{{isset($arr[0])?$arr[0]:null}}" required>
			</div>
			<div class='form-group'>
			...
			<div class='form-group'>
				...
				<input type="reset" name="reset" value='Reset' class='btn btn-secondary px-5'>
			</div>
		</form>
	</div>
</body>
